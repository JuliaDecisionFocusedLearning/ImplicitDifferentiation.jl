...
linear_solver_candidates = (\, first ∘ Krylov.gmres)
backend_candidates = (nothing, AutoForwardDiff(; chunksize=1));
x_candidates = (rand(Float32, 2), rand(Float64, 2));

## Test loop

@testset "$linear_solver - $(typeof(backend)) - $(typeof(x))" for (
    linear_solver, backend, x
) in Iterators.product(
    linear_solver_candidates, backend_candidates, x_candidates
)
    @info "$linear_solver - $(typeof(backend)) - $(typeof(x))"
    test_implicit(
        x; linear_solver, conditions_x_backend=backend, conditions_y_backend=backend
    )
end
