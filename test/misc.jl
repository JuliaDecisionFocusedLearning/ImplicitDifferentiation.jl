...
function get_implicit_func(::Val{byproduct}) where {byproduct}
    if byproduct
        forward = (x) -> (mysqrt(x), 0)
        conditions = (x, y, z) -> y .^ 2 .- x
        implicit = ImplicitFunction(forward, conditions, Val(true))
    else
        forward = (x) -> mysqrt(x)
...
@testset "byproduct = $byproduct" for byproduct in (Val(true), Val(false))
    implicit = get_implicit_func(byproduct)
